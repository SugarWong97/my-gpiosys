...
static int str_to_lower(char * str, int len)
{
    int i;
    for(i=0;i<len;i++)//遍历
    {
 ...
 *          *
 *           **********************************************/
static void str_replace(char * pszInput, char * pszOld, char * pszNew,char * pszOutput,int nOutputlen)
{
    char temp[128];
    int nLen=0;
    char *s, *p;
...
int str_to_rk_gpio(const char * str)
{
    ...
    printf("pin = %d \t= [ bank(%d) * 32 ] + [ group(%c->%d) * 8 ] + pos(%d)\n", pin, bank, group, group - 'a', pos);
